...
detectable in *individual interactions* even though coevolution fails to
...
![Correlation (Pearson's statistic) between the interaction-level contribution to coevolutionary signal at the local and continental scales. Each dot represents one community. Communities in the shaded area have non-significant correlations ($\alpha = 0.05$).](../figures/figure2.pdf)
