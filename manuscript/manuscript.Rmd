...
their ecological stability) and its contribution to the co-phylogenetic process
(which can be a proxy of their evolutionary stability) draws into question
the basic long-term stability of interactions. Specifically, the existence of
interactions with strong co-phylogenetic contribution, but low probability, is
puzzling. It is tempting to hypothesize that these interactions may have once been strong
enough to drive (co-)speciation, but are now decaying, due either to the fact
that the host-parasite pair has been disrupted or as a consequence of
ecological processes. DBS Can you spell this transition out for me better? I don't quite see the link. DBS @janz11err show that parasites can frequently acquire
...
from ecological contact between distantly related species. This emphasizes the
...
of the ancestral range of the species can shed some light on whether acquisition
...
happens after the other; host shift, where the parasite acquires a new host
(and can subsequently lose the previous host, undergo speciation, or maintain its
ability to use the previous host); and host-range expansion, in which the parasite
acquires a novel host.
DBS Are we sure the differences between host shift and host-range expansion clear enough in the above? DBS
When the likelihood of both host and parasite extinction
events are added to these models, the variety of outcomes is considerable.
...
that the number of cospeciation events is inflated by cophenetic methods like
*PACo* or *ParaFit* [although @lope01mpf reported an increased number of
cospeciations when accounting for time of speciation events]. This reinforces
the need for high-quality phylogeny data calibrated for communities in which the
interactions are known. Not only will this help us make more robust inference about
the evolution of networks [@eklo16pcf], it will also help us leverage information about
species interactions embedded within networks to better understand coevolutionary
dynamics.
