...
{\bfseries\huge Interactions retain the co-phylogenetic matching that communities lost}
...
~\emph{1,2,3}
\quad
\mbox{Daniel B. Stouffer}
~\emph{1}
\end{flushleft}
\bigskip

\small
\textbf{1}:~Centre for Integrative Ecology, School of Biological Sciences,
University of Canterbury, Christchurch, New Zealand;\quad\textbf{2}:~Université de Montréal, Département de Sciences Biologiques, Montréal,
Canada;\quad\textbf{3}:~Québec Centre for Biodiversity Sciences, Montréal, Canada\normalsize
\vfill
...
species interactions  - host-parasites  - phylogenetic congruence  - network cophylogeny  - community phylogenetics \vskip 4em
}
...
\xrefname{table}\cref{tbl:contingency}; \xrefname{fig.}\cref{fig:maps}).
...
local and regional communities (\xrefname{fig.}\cref{fig:relative}), we
...
(\(\rho = 0.95\); \xrefname{fig.}\cref{fig:contributions}A).
...
quantified (\xrefname{fig.}\cref{fig:contributions}B). Because
...
\(\alpha = 0.05\); \xrefname{fig.}\cref{fig:consistency}). Recall that
...
(\xrefname{fig.}\cref{fig:scales}). Remarkably, this result implies that
